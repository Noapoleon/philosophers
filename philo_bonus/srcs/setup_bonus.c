...
/*   Updated: 2023/03/13 05:16:06 by nlegrand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	sim_setup(t_data *data, int ac, char **av)
{
	...
	if (get_rules(&data->rules, ac, av) == -1)
		return (-1);
	data->pids = malloc(sizeof(pid_t) * data->rules.num_philos);
	if (data->pids == NULL)
		return (-1);
	if (make_sems(data) == -1)
		return (printf(PHILO_ERR PE_SEMS), free(data->pids), -1);
	data->philo.meals = 0;
	return (0);
}
...
int	make_sems(t_data *data)
{
	...
	if (data->sync_sem == SEM_FAILED)
		return (sem_close(data->forks_sem), unlink_sems(), -1);
	data->print_sem = sem_open("/print_sem", O_CREAT, 0644, 1);
	if (data->print_sem == SEM_FAILED)
		return (sem_close(data->forks_sem), sem_close(data->sync_sem),
			unlink_sems(), -1);
	data->forking_sem = sem_open("/forking_sem", O_CREAT, 0644, 1);
	if (data->forking_sem == SEM_FAILED)
		return (sem_close(data->forks_sem), sem_close(data->sync_sem),
			sem_close(data->print_sem), unlink_sems(), -1);
	data->ate_sem = sem_open("/ate_sem", O_CREAT, 0644, 0);
	if (data->ate_sem == SEM_FAILED)
		return (sem_close(data->forks_sem), sem_close(data->sync_sem),
			sem_close(data->print_sem), sem_close(data->forking_sem),
			unlink_sems(), -1);
	return (0);
}
